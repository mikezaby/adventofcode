class Bag
  ...
  def all_children_count
    @all_children_count ||= children.sum do |child|
      child[:count] + child[:count] * child[:bag].all_children_count
    end
  end

  def add_child(child, count)
    child.parents.push(self)
    children.push({ bag: child, count: count })
  end
  ...
  def read_rules(data)
    data.map(&:strip).each do |line|
      ...
      parse_children(rule[4..-1]).each do |cname, count|
        bags[name].add_child(bags[cname], count)
      end
    end
  end
...
@output = rules.bags['shiny gold'].all_children_count
